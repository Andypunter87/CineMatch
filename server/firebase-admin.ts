...
export async function createFirebaseToken(
  ...
  try {
    ...
    if (isFirebaseAdminInitialized && firebaseApp) {
      ...
      console.warn(
        "Firebase Admin not initialized, using fallback token system",
      );
  ...
  try {
    ...
    return {
      ...
      diagnostics: {
        ...
        tokenSystem: isFirebaseAdminInitialized
          ? "Firebase Admin SDK"
          : "Fallback token",
        firebaseProject: projectId,
      },
...
export async function saveOnboardingRatings(
  ...
  try {
    ...
    console.log(
      `Saved ${ratings.length} onboarding ratings to Firestore for user ${uid}`,
    );
    return { success: true };
  } catch (error) {
